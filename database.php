...
class Database{
    ...
    public function editReservasi($id) {
        $data = mysqli_query($this->koneksi,"select * from reservasi where id='$id'");
    ...
    function updateReservasi($id, $nama_pelanggan, $keluhan, $status, $tanggal){
		mysqli_query($this->koneksi, "UPDATE reservasi SET nama_pelanggan='$nama_pelanggan', keluhan='$keluhan', status='$status', tanggal='$tanggal' WHERE id='$id'");
	}

    function hapusReservasi($id){
		mysqli_query($this->koneksi,"delete from reservasi where id='$id'");
	}

    function tampilLaporan() {
        $data =mysqli_query($this->koneksi, "SELECT * FROM laporan");
        while($dat=mysqli_fetch_array($data)) {
            $hasil [] = $dat;
        }
        return $hasil;
    ...
    function hapusLaporan($id){
		mysqli_query($this->koneksi,"delete from laporan where id='$id'");
	}
    ...
    function editPelanggan($id) {
        $data = mysqli_query($this->koneksi,"select * from pelanggan where id='$id'");
		while($d = mysqli_fetch_array($data)){
			$hasil[] = $d;
		}
		return $hasil;
    ...
    function updatePelanggan($id, $nama_pelanggan, $jk, $hp, $email, $alamat){
		mysqli_query($this->koneksi, "UPDATE pelanggan SET nama_pelanggan='$nama_pelanggan', jk='$jk', hp='$hp', email='$email', alamat='$alamat' WHERE id='$id'");
	}
